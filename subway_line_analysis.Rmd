...
```{r, warning=FALSE, fig.cap="Two different visualizations of delay frequency distribution by subway line: The Stacked bar chart reveals the phenomenon that riders on most lines report delays a few times a week, though the share of everyday delays varies widely. The delay frequency is on J and 7 lines. Use the interactive hover label to view exact proportions for clearer line-by-line comparisons. The heat map highlights system-wide patterns, showing consistently high weekly delay reports and notable everyday delays on lines like the 7. Hover details reveal the precise counts and proportions underlying each cell."}

# Distribution of Delay Frequency Responses -- stacked bar chart

dist_delay <- scored_data |>
  filter(!is.na(frequency_of_delays)) |>
  count(subway_line, group_color, frequency_of_delays, name = "n") |>
  group_by(subway_line, group_color) |>
  mutate(prop = n / sum(n)) |>
  ungroup() |>
  mutate(
...
dist_delay_plot <- plot_ly(
  ...
  hovertemplate = paste(
    "Subway Line: %{x}<br>",
    "Line Group: %{customdata}<br>",
    "Proportion of Riders: %{y:.1%}<br>",
    "<extra></extra>"
  )
) |>
  layout(
    barmode = "stack",
    title = list(text = "Distribution of Delay Frequency Responses by Subway Line", x = 0.5),
    xaxis = list(
      title = "Subway Line",
      tickangle = -45,
      tickmode = "array",
      tickvals = unique(dist_delay$subway_line),
      ticktext = unique(dist_delay$subway_line)
    ),
