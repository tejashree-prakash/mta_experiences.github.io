...
<h3>Which subway line is the most used?</h3>
...
<h3>Why are they taking the subway?</h3>
...
```
<br>
<h3>How long do people stay on the train?</h3>
...
<h3>What are the top complaints?</h3>

```{r,fig.width=9, fig.height=6, fig.cap="Delays and overcrowded trains dominate in every borough, with Brooklyn and Manhattan reporting the highest volumes overall. Meanwhile, issues like crime/safety and old trains appear less frequently but still show notable variation across boroughs."}
...
rider_data_full_clean <- rider_data_full %>%
  ...
  # Split and clean complaints
  mutate(
    complaint = str_split(top_three_complaints, ",\\s*")
  ) %>%
  tidyr::unnest(complaint) %>%
  mutate(
    complaint = str_trim(complaint),  # Clean whitespace
    complaint = case_when(
      # Standardize complaint names if needed
  ...
  filter(!is.na(borough), !is.na(complaint), complaint != "")

# Get top complaints across all boroughs
all_top_complaints <- rider_data_full_clean %>%
  count(complaint, sort = TRUE) %>%
  slice_head(n = 8) %>%  # Get top 8 complaint types overall
  pull(complaint)

# Filter and prepare data for plotting
top_complaints_borough <- rider_data_full_clean %>%
  filter(complaint %in% all_top_complaints) %>%
  count(borough, complaint) %>%
  complete(borough, complaint, fill = list(n = 0)) %>%
  # Reorder complaints by total frequency
  mutate(
    complaint = factor(complaint, levels = rev(all_top_complaints))
  )
...
plot_top_borough <- top_complaints_borough |> 
  plot_ly(
    ...
    colors = viridis::viridis_pal()(length(valid_boroughs))
  ) |> 
  layout(
...
<h3>Do people think the subway is affordable?</h3>
...
```
<br>
<h3>Is it fair to increase fares?</h3>
...
<h3>Now that we know this information, what do the people overall think about their MTA experince?</h3>
