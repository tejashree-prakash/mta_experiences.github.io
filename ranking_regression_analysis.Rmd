...
```{r, fig.cap="We began our regression analysis by visualizing the overall satisfaction levels across our sample. Understanding this provides a baseline for our regression results, highlights the prevalence of positive, neutral, and negative perceptions, and helps identify patterns or imbalances in rider satisfaction. This step ensures that our modeling is grounded in a clear understanding of the data, allowing us to effectively assess how different predictors like borough and ride length relate to overall satisfaction. We observe that the graph demonstrates a slightly skewed, but close to normal distribution of overall satisfaction responses."}
...
```{r, fig.cap="The residual plot from the linear regression model is highly ordered and is heterscedastic. Therefore, we decided to proceed with a logistic regression model instead."}
#Visualizations
regression_rider_data |> 
...
<br>
<h3>Running a Logistic Regression Model Instead</h3>
...
```
<br>To run a logistic regression model, we ordered our overall satisfaction variable to maintain its ordinal status. Then, we ran a *clm model*, a type of logistic regression that is specific to ordinal outcomes.<br><br>We see from our regression results here that all of our predictors were significant at the 5% level, as shown by the bold. Due to the nature of the clm model, linear, quadratic, and cubic polynomial scale effects were performed for each predictor that was ordinal, as we see in this model output. These different scale effects represent different potential trends in how the ordered predictor affects the outcome. A significant linear coefficient suggests a consistent directional change across levels, whereas a significant quadratic coefficient suggests an acceleration or deceleration curve, and a cubic coefficient suggests a complex, S-shaped curve. <br><br>Borough had the greatest odds ratio [OR: 1.14, CL: (1.03-1.26)] relative to the other predictors, indicating it has the largest influence on overall satisfaction relative to the other predictors. This was followed by average length of subway ride [OR: 0.66, CL: (0.50-0.88)], frequency of rerouting [OR: 0.53, CL: (0.46-0.61)], approximate delay duration [OR: 0.36, CL: (0.24-0.55)], then frequency of delays [OR: 0.01, CL: (0.00-0.03)].

<br>
<h3>Visualizing Effect Size</h3>

<h4>Do predictors increase or decrease satisfaction odds?</h4>
```{r, fig.cap="All of the significant predictors increase the odds of experiencing satisfaction, which makes sense since we ordered our variables from the lowest to highest level appropriate to the survey answers.<br><br>As we continue our analysis, we chose to focus on the linear scale effects of each ordinal predictor, as they are more interpretable and provide a clearer understanding of the directional relationship between each predictor and overall satisfaction."}
...
<h3>Predicting Probability of Satisfaction by Linearity<h3> 
<h4>How satisfied is a rider based on the length of their subway ride?</h4>

```{r, fig.cap="We chose to represent prediction with the two predictors that are significant at the linear level, indicating a proportional relationship that can be represented through a prediction heat map. This plot allows us to understand how likely the probability of a very satisfied outcome is based on what borough a respondent is from and the average length of subway ride. Regardless of borough, shorter subway rides tend to have a greater chance of being more satisfied!"}
...
<br>
<h4>How satisfied is a rider based on the frequency of delays?</h4>
```{r, fig.cap="Similar to the last plot, we wanted to see how this prediction is represented for frequency delay data. Across all boroughs, the shorter the frequency delay, the higher the probability of satisfaction! Makes sense!"}
