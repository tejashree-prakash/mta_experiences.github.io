...
# Chart A - Complaint types by time and line overall 
...
# Build one trace per complaint type * time_of_day
plot <- plot_ly()
...
for (t in times) {
  ...
  for (comp in unique(df_t$complaint)) {
    ...
    plot <- plot |> add_trace(
      ...
      orientation = "h",
      name = comp,
      # Use hovertemplate with customdata for count
      hovertemplate = paste(
        "Time: ", t,
        "<br>Line: %{y}",
        "<br>Complaint: ", comp,
      ...
      customdata = ~n,  # Pass the count data
...
###  Distribution of complaint types overall 

```{r, fig.width=15, fig.height=8}

# Chart C - Distribution of complaint types overall 
...
```{r,fig.width=15, fig.height=8}
#  Chart E - Complaint types by line and borough
